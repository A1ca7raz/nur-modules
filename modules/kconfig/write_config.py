# https://github.com/nix-community/plasma-manager/blob/d226a67fdf835be15e5270c36406e922f4b6fa84/script/write_config.py
import glob
import json
import os
import re
...
class ConfigValue:
    ...
    def to_line(self, key: str) -> str:
        ...
        _key = escape(key) if self.escapeKey else key
        key = _key + self.marking
...
def write_configs(d: dict[str, Any], filepath: str):
    config = KConfManager(
        filepath, d
    )
    config.run()
...
def main():
    if len(sys.argv) != 3:
        raise ValueError(
            f"Must receive exactly 3 arguments, got: {len(sys.argv) - 1}"
        )
